...
{
    LEDS_CONFIGURE(ADVERTISING_LED_PIN | CONNECTED_LED_PIN | LEDBUTTON_LED_PIN);//配置LED
    LEDS_OFF(ADVERTISING_LED_PIN | CONNECTED_LED_PIN | LEDBUTTON_LED_PIN);//关掉LED
}
...
{
    on_ble_evt(p_ble_evt);//服务分成一块一块的，每种服务只关心他自己的状态
    ble_conn_params_on_ble_evt(p_ble_evt);
    ble_lbs_on_ble_evt(&m_lbs, p_ble_evt);//LED 服务
}
...
int main(void)
{
    ...
    conn_params_init();//连接初始化

    // Start execution.
    advertising_start();//广播初始化
